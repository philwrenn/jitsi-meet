...
export default class AlwaysOnTop extends Component<*, State> {
     ...
     * @returns {void}
     */
    _avatarChangedListener({ avatar, id }) {
     ...
     * @returns {void}
     */
    _displayNameChangedListener({ displayname, id }) {
        if (api._getOnStageParticipant() !== id) {
            return;
        }
    ...
    _largeVideoChangedListener() {
        ...
        const avatar = api.getAvatar(userID);
    ...
    _renderVideoNotAvailableScreen() {
        const { avatar, displayName, isVideoDisplayed } = this.state;
